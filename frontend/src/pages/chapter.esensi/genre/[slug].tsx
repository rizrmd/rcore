...
import { FilterIcon, BookOpenIcon, HeartIcon } from "lucide-react";
...
export default (data: Awaited<ReturnType<typeof api.genre>>["data"]) => {
  ...
  const genreSlug = data?.selected_genre || "";
  const genreName = data?.genre_info?.name || "";
  const genre = data?.genre_info;

  // If genre not found, show error message (but not during navigation)
  if (!isNavigating && !genre) {
    return (
      <EsensiChapterLayout>
            ...
            <div className="pt-8 pb-6">
              <div className="text-center">
                ...
                <p className="text-lg text-gray-600 max-w-2xl mx-auto mb-6">
                  Maaf, genre yang Anda cari tidak ditemukan atau telah dihapus.
                </p>
  ...
  const handleSearch = async () => {
    ...
    navigate(
      `/genre/${genreSlug}${params.toString() ? "?" + params.toString() : ""}`
    );
  };
  ...
  const handlePageChange = (page: number) => {
    ...
    navigate(`/genre/${genreSlug}?${params.toString()}`);
  };
            ...
            <div className="flex items-center gap-4 mx-auto">
              ...
              <div className="text-white">
                <h1 className="text-2xl font-bold mb-2">
                  {genreName ? (
                    <>
                      Berikut semua bacaan dengan genre{" "}
                      <span className="text-(--esensi-color-alt)">
                        {genreName}
                      </span>
                    </>
                  ) : (
                    <>Bacaan dengan genre terpopuler</>
                  )}
                </h1> 
                <p className="text-white/90 text-base">
                  {genreName
                    ? `Cek semua judul yang bergenre ${genreName.toLowerCase()} berikut dan masukkan sebagai daftar bacamu`
                    : "Temukan koleksi buku bergenre pilihan dan masukkan sebagai daftar bacamu"}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="esensi-container mx-auto">
          ...
          <div className="flex flex-col lg:flex-row gap-6 pt-6">
            ...
            <FilterSection
              ...
              onReset={handleReset}
              genreInfo={{ slug: genreSlug, name: genreName }}
