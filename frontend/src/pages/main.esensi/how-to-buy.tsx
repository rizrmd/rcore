import { BannerSlider } from "@/components/esensi/ui/banner-slider";
...
export default (data: Awaited<ReturnType<typeof api.howtobuy>>["data"]) => {
  ...
  const tesBanner = (<BannerSlider data={`tes aja`}/>);
  return (
    <MainEsensiLayout header_config={header_config} mobile_menu={true}>
     <div className="relative w-full h-auto">{tesBanner}</div> 
    </MainEsensiLayout>
  );
};
